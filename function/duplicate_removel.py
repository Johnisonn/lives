...
def remove_dump_urls(chs_dict):
    ...
    logger.info('—' * 100)
    logger.info('【开始对url地址去重】'.center(100))
    ...
    logger.info(f'共去除重复频道 {dump_chs_count} 个，去除重复url地址 {dump_urls_count} 个'.center(100))
    logger.info(f'去重后剩余频道 {ch_count} 个，剩余url地址 {len(urls_set)} 个'.center(100))
    return chs_dict

def remove_dump_names(names_dict):
# 对不同分类内重名频道名称去重
    names_set = set()
    to_remove = []
    ...
    logger.info('—' * 100)
    logger.info('开始对所有分类中同名频道名称去重...'.center(100))
    logger.info('—' * 100)
    ...
    for cate, name in to_remove:
        ...
        logger.info(f'分类【{cate}】中【{name}】名称重复已去重'.center(100))
        if len(names_dict[cate]) < 1:
            del names_dict[cate]
            logger.info(f'分类【{cate}】重复已删除'.center(100))
            dump_cates_count += 1

    logger.info(f'共去除重复频道名 {dump_names_count} 个'.center(100))
    logger.info(f'去除空分类 {dump_cates_count} 个'.center(100))
    logger.info(f'去重后共剩余频道名称 {len(names_set)} 个！'.center(100))
